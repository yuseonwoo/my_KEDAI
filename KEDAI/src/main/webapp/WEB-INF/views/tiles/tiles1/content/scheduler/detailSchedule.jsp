...
<script type="text/javascript">
	...
	function delSchedule(scheduleno){
		...
		if(bool){
			$.ajax({
				url: "<%= ctxPath%>/scheduler/deleteSchedule.kedai",
				type: "post",
	...
	function editSchedule(scheduleno){
		...
		frm.action = "<%= ctxPath%>/scheduler/editSchedule.kedai";
		frm.method = "post";
		frm.submit();
	}
...
<h3 style="display: inline-block;">일정 상세보기</h3>&nbsp;&nbsp;<a href="<%= ctxPath%>/scheduler.kedai"><span>◀캘린더로 돌아가기</span></a> 

		<table id="schedule" class="table table-bordered">
			<tr>
				<th style="width: 160px; vertical-align: middle;">일자</th>
				<td>
					<span id="startdate">${requestScope.map.CAL_STARTDATE}</span>&nbsp;~&nbsp;<span id="enddate">${requestScope.map.CAL_ENDDATE}</span>&nbsp;&nbsp;  
					<input type="checkbox" id="allDay" disabled/>&nbsp;종일
				</td>
			</tr>
			<tr>
				<th style="vertical-align: middle;">제목</th>
				<td>${requestScope.map.CAL_SUBJECT}</td>
			</tr>
			...
			<tr>
				...
				<td>${requestScope.map.CAL_PLACE}</td>
			</tr>
			
			<tr>
				<th style="vertical-align: middle;">공유자</th>
				<td>${requestScope.map.CAL_JOINUSER}</td>
			</tr>
			<tr>
				<th style="vertical-align: middle;">내용</th>
				<td><textarea id="content" rows="10" cols="100" style="height: 200px; border: none;" readonly>${requestScope.map.CAL_CONTENT}</textarea></td>
			</tr>
			<tr>
	...
	<c:set var="v_fk_empid" value="${requestScope.map.FK_EMPID}" />
	<c:set var="v_fk_lgcatgono" value="${requestScope.map.FK_LGCATGONO}"/>
	<c:set var="v_loginuser_empid" value="${sessionScope.loginuser.empid}"/>
	...
	<div style="float: right;">
	    ...
	    --%>
			<c:if test="${v_fk_lgcatgono eq '2' && sessionScope.loginuser.fk_job_code == '1' }">
			...
			<c:if test="${v_fk_lgcatgono eq '1' && v_fk_empid eq v_loginuser_empid}">
				<button type="button" id="edit" class="btn_normal" onclick="editSchedule('${requestScope.map.SCHEDULENO}')">수정</button>
				<button type="button" class="btn_normal" onclick="delSchedule('${requestScope.map.SCHEDULENO}')">삭제</button>
			</c:if>
	    ...
	    --%>
			...
			<c:if test="${v_fk_lgcatgono eq '1' && v_fk_empid eq v_loginuser_empid}">
				...
				<button type="button" id="cancel" class="btn_normal" style="margin-right: 0px; background-color: #990000;" onclick="javascript:location.href='<%= ctxPath%>/scheduler/detailSchedule.kedai'">취소</button> 
		</c:if>
