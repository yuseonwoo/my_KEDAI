...
<script type="text/javascript">
	...
	//  $('input#toDate').datepicker('setDate', 'today'); //(-1D:하루전, -1M:한달전, -1Y:일년전), (+1D:하루후, +1M:한달후, +1Y:일년후)	
	    ...
	    if(${not empty requestScope.paraMap}){
	    	  $("input[name=cal_startdate]").val("${requestScope.paraMap.cal_startdate}");
	    	  $("input[name=cal_enddate]").val("${requestScope.paraMap.cal_enddate}");
	...
	function goDetail(scheduleno){
		...
		frm.method="get";
		frm.action="<%= ctxPath%>/scheduler/detailSchedule.kedai";
		frm.submit();
...
<div style="margin-left: 80px; width: 88%;">
	<div>
		<h3 style="display: inline-block;">일정 검색결과</h3>&nbsp;&nbsp;<a href="<%= ctxPath%>/scheduler.kedai"><span>◀캘린더로 돌아가기</span></a>
			...
			<c:if test="${not empty requestScope.scheduleList}">
				<c:forEach var="map" items="${requestScope.scheduleList}">
					<tr class="infoSchedule">
						...
						<td>${map.CAL_STARTDATE} - ${map.CAL_ENDDATE}</td>
						<td>${map.LGCATGONAME} - ${map.SMCATGONAME}</td>
						<td>${map.NAME}</td>  <%-- 캘린더 작성자명 --%>
						<td>${map.CAL_SUBJECT}</td>
						<td>${map.CAL_CONTENT}</td>
					</tr>
				</c:forEach>
			</c:if>
		</tbody>
