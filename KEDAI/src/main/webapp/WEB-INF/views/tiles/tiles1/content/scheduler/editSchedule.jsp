	...
	$(document).ready(function(){
	 ...
	 //	console.log(end_hour);
		...
		$("select.calType").change(function(){
			...
			var fk_empid = $("input[name=fk_empid]").val();  // 로그인 된 사용자아이디
		...
		$("input#joinUserName").bind("keyup",function(){
			...
			//	console.log("확인용 joinUserName : " + joinUserName);
				$.ajax({
					url:"<%= ctxPath%>/scheduler/insertSchedule/searchJoinUserList.kedai",
						...
						if(json.length > 0){
							...
							$.each(json, function(index,item){
								...
								if(name.includes(joinUserName)){ // name 이라는 문자열에 joinUserName 라는 문자열이 포함된 경우라면 true , 
								   ...
								   joinUserArr.push(name+"("+item.empid+")");
								}
							});
		...
		$("button#edit").click(function(){
			...
			$("input[name=cal_startdate]").val(sdate);
			$("input[name=cal_enddate]").val(edate);
		...
		<table id="schedule" class="table table-bordered">
			<tr>
				...
				<td>
					...
					<input type="hidden" name="cal_startdate"/>
					<input type="hidden" name="cal_enddate"/>
				</td>
			</tr>
			<tr>
				<th>제목</th>
			...
			<tr>
				...
				<input type="hidden" name="cal_joinuser"/>
				</td>
			</tr>
			<tr>
				<th>내용</th>
