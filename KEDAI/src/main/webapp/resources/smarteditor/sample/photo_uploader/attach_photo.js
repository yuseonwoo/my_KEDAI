		...
		try{
			if( !oNavigator.ie ){
				if(!!oNavigator.safari && oNavigator.version <= 5){
					...
					bSupportDragAndDropAPI = true; //default 값은 bSupportDragAndDropAPI = false; 이다.
				}
			} else {
				bSupportDragAndDropAPI = false;
			}
    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/KEDAI/image/multiplePhotoUpload.kedai'; 	// upload URL => 파이널 프로젝트시 /board 를 프로젝트의 컨텍스패스명으로 수정하기!!!
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/KEDAI/image/photoUpload.kedai',	// 파일업로드시 동작되는 컨트롤러 url  
